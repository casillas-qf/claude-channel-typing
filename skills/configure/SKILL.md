...
allowed-tools:
  ...
  - Bash(mkdir *)
  - Bash(chmod *)
  - Bash(printenv TELEGRAM_STATE_DIR)
---
...
**IMPORTANT: Before doing anything, determine the state directory.**

Run: `printenv TELEGRAM_STATE_DIR`

- If it returns a path → use that as `STATE_DIR`
- If empty/unset → use `~/.claude/channels/telegram` as `STATE_DIR`
...
1. **State directory** — show which directory is being used (default or custom via TELEGRAM_STATE_DIR).

2. **Token** — check `STATE_DIR/.env` for
   `TELEGRAM_BOT_TOKEN`. Show set/not-set; if set, show first 10 chars masked
   (`123456789:...`).

3. **Access** — read `STATE_DIR/access.json` (missing file
